...
<meta http-equiv='refresh' content='0; url=login.php'>
	...
	$query="SELECT c.c_id,c.c_name,c.c_tel FROM company c,(SELECT rownum AS rnum,c_id FROM company WHERE c_ok IS NOT NULL) r WHERE c.c_id=r.c_id AND c.c_ok IS NOT NULL AND r.rnum BETWEEN $record_start+1 AND $record_start+$record_count";
	$stmt=OCIParse($conn,$query);
	OCIExecute($stmt);
...
<html>
<hrad>
<title>승인된 기업 리스트</title>
</head>
<body>

<center>
<!- 글 개수와 페이지 번호 출력 ->
<table border='0' width='306' style='table-layout:fixed;'>
...
<table border='0' width='300' style='table-layout:fixed;'>
	<tr height='20' bgcolor='#eef0f4'>
		<td width='100' align='center'>
			<font size='2' face='HY나무M'><b>ID</b></font>
		</td>
		<td width='100' align='center'>
...
if($total_record>$record_count) {
	...
	if(($total_page>1) && ($record_start!=0)) {
		...
		echo("
<a href='company_list.php?record_start=$pre_page_start'><font size='3'>◀</font></a>&nbsp;&nbsp;&nbsp;
		");
	}
	...
	if($current_page*$record_count > $record_count*$link_count) {
		...
		echo("
<a href='company_list.php?record_start=$pre_group_start'>[이전 $link_count 개]</a>&nbsp;&nbsp;&nbsp;
		");
	}
	...
	for($i=0;$i<$link_count;$i++) {
		...
		if($input_start<$total_record) {
			if($input_start!=$record_start) {
				echo("
<a href='company_list.php?record_start=$input_start'><font size='3'>$link</font></a>&nbsp;&nbsp;
				");
			} else {
				echo("
<font size='3' color='silver'>$link</font>&nbsp;&nbsp;
	...
	if($total_record>(($group+1)*$record_count*$link_count)) {
		...
		echo("
<a href='company_list.php?record_start=$next_group_start'>[다음 $link_count 개]</a>
		");
	}
	...
	if(($total_page>1) && ($record_start!=($total_page*$record_count-$record_count))) {
		...
		echo("
&nbsp;&nbsp;&nbsp;<a href='company_list.php?record_start=$next_page_start'><font size='3'>▶</font></a>
		");
	}
}
	echo("
...
<table>
	<tr>
		<td align='center'>
			<form action='company_list.php?record_start=$record_start' method='post'>
			<select name='field'>
