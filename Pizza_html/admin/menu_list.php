...
<meta http-equiv='refresh' content='0; url=login.php'>
	...
	//게시판 조회 질의
	$query="SELECT m_id,m_name,m_price,m_size FROM menu";
	...
	$query="SELECT m.m_id,m.m_name,m.m_price,m.m_size FROM menu m,(SELECT rownum AS rnum,m_id FROM menu) r WHERE m.m_id=r.m_id and r.rnum BETWEEN $record_start+1 AND $record_start+$record_count";
	$stmt=OCIParse($conn,$query);
	OCIExecute($stmt);
...
<html>
<hrad>
<title>메뉴항목 리스트</title>
</head>
<body>

<center>
<!- 글 개수와 페이지 번호 출력 ->
<table border='0' width='470' style='table-layout:fixed;'>
...
<table border='0' width='470' style='table-layout:fixed;'>
	<tr height='20' bgcolor='#eef0f4'>
		<td width='100' align='center'>
			<font size='2' face='HY나무M'><b>M_ID</b></font>
		</td>
		<td width='200' align='center'>
			<font size='2' face='HY나무M'><b>M_NAME</b></font>
		</td>
		<td width='100' align='center'>
			<font size='2' face='HY나무M'><b>M_PRICE</b></font>
		</td>
		<td width='70' align='center'>
			<font size='2' face='HY나무M'><b>M_SIZE</b></font>
		</td>
	</tr>
	");
...
if($total_record>$record_count) {
	...
	if(($total_page>1) && ($record_start!=0)) {
		...
		echo("
<a href='menu_list.php?record_start=$pre_page_start'><font size='3'>◀</font></a>&nbsp;&nbsp;&nbsp;
		");
	}
	...
	if($current_page*$record_count > $record_count*$link_count) {
		...
		echo("
<a href='menu_list.php?record_start=$pre_group_start'>[이전 $link_count 개]</a>&nbsp;&nbsp;&nbsp;
		");
	}
	...
	for($i=0;$i<$link_count;$i++) {
		...
		if($input_start<$total_record) {
			if($input_start!=$record_start) {
				echo("
<a href='menu_list.php?record_start=$input_start'><font size='3'>$link</font></a>&nbsp;&nbsp;
				");
			} else {
				echo("
<font size='3' color='silver'>$link</font>&nbsp;&nbsp;
	...
	if($total_record>(($group+1)*$record_count*$link_count)) {
		...
		echo("
<a href='menu_list.php?record_start=$next_group_start'>[다음 $link_count 개]</a>
		");
	}
	...
	if(($total_page>1) && ($record_start!=($total_page*$record_count-$record_count))) {
		...
		echo("
&nbsp;&nbsp;&nbsp;<a href='menu_list.php?record_start=$next_page_start'><font size='3'>▶</font></a>
		");
	}
}
	echo("
...
<table>
	<tr>
		<td align='center'>
			<form action='menu_list.php?record_start=$record_start' method='post'>
			<select name='field'>
