...
<meta http-equiv='refresh' content='0; url=login.php'>
	...
	$query="SELECT TO_CHAR(po.po_orderdate,'YYYY-MM-DD'),m.m_id,m.m_name,m.m_size,po.po_ordervolume,(m.m_price*po.po_ordervolume),po.po_paymethod,po.po_delivery_state,p.p_id,p.p_name,p.p_addr,p.p_tel FROM person p, pizza_order po,(SELECT po_orderdate,rownum AS rnum FROM pizza_order) r, menu m WHERE p.p_id=po.p_id AND po.po_orderdate=r.po_orderdate AND po.m_id=m.m_id AND r.rnum BETWEEN $record_start+1 AND $record_start+$record_count";
	$stmt=OCIParse($conn,$query);
	OCIExecute($stmt);
...
<!- 레코드 제목 출력 ->
<table border='0' width='1210' style='table-layout:fixed;'>
	<tr height='20' bgcolor='#eef0f4'>
		<td width='100' align='center'>
			<font size='2' face='HY나무M'><b>날짜</b></font>
		</td>
		<td width='100' align='center'>
			<font size='2' face='HY나무M'><b>메뉴ID</b></font>
		</td>
		...
		<td width='100' align='center'>
			<font size='2' face='HY나무M'><b>고객ID</b></font>
		</td>
		<td width='100' align='center'>
			<font size='2' face='HY나무M'><b>고객이름</b></font>
		</td>
		<td width='150' align='center'>
			<font size='2' face='HY나무M'><b>고객주소</b></font>
		</td>
		<td width='100' align='center'>
			<font size='2' face='HY나무M'><b>고객연락처</b></font>
		</td>
	</tr>
	");
...
if($total_record>$record_count) {
	...
	if(($total_page>1) && ($record_start!=0)) {
		...
		echo("
<a href='pizza_order_list.php?record_start=$pre_page_start'><font size='3'>◀</font></a>&nbsp;&nbsp;&nbsp;
		");
	}
	...
	if($current_page*$record_count > $record_count*$link_count) {
		...
		echo("
<a href='pizza_order_list.php?record_start=$pre_group_start'>[이전 $link_count 개]</a>&nbsp;&nbsp;&nbsp;
		");
	}
	...
	for($i=0;$i<$link_count;$i++) {
		...
		if($input_start<$total_record) {
			if($input_start!=$record_start) {
				echo("
<a href='pizza_order_list.php?record_start=$input_start'><font size='3'>$link</font></a>&nbsp;&nbsp;
				");
			} else {
				echo("
<font size='3' color='silver'>$link</font>&nbsp;&nbsp;
	...
	if($total_record>(($group+1)*$record_count*$link_count)) {
		...
		echo("
<a href='pizzs_order_list.php?record_start=$next_group_start'>[다음 $link_count 개]</a>
		");
	}
	...
	if(($total_page>1) && ($record_start!=($total_page*$record_count-$record_count))) {
		...
		echo("
&nbsp;&nbsp;&nbsp;<a href='pizza_order_list.php?record_start=$next_page_start'><font size='3'>▶</font></a>
		");
	}
}
	echo("
...
<table>
	<tr>
		<td align='center'>
			<form action='pizza_order_list.php?record_start=$record_start' method='post'>
			<select name='field'>
