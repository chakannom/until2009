...
<meta http-equiv='refresh' content='0; url=login.php'>
	...
	//게시판 조회 질의
	$query="SELECT * FROM delivery_supply WHERE c_id='$com_id'";
...
if($total_record>$record_count) {
	...
	if(($total_page>1) && ($record_start!=0)) {
		...
		echo("
<a href='delivery_supply_list.php?record_start=$pre_page_start'><font size='3'>◀</font></a>&nbsp;&nbsp;&nbsp;
		");
	}
	...
	if($current_page*$record_count > $record_count*$link_count) {
		...
		echo("
<a href='delivery_supply_list.php?record_start=$pre_group_start'>[이전 $link_count 개]</a>&nbsp;&nbsp;&nbsp;
		");
	}
	...
	for($i=0;$i<$link_count;$i++) {
		...
		if($input_start<$total_record) {
			if($input_start!=$record_start) {
				echo("
<a href='delivery_supply_list.php?record_start=$input_start'><font size='3'>$link</font></a>&nbsp;&nbsp;
				");
			} else {
				echo("
<font size='3' color='silver'>$link</font>&nbsp;&nbsp;
	...
	if($total_record>(($group+1)*$record_count*$link_count)) {
		...
		echo("
<a href='delivery_supply_list.php?record_start=$next_group_start'>[다음 $link_count 개]</a>
		");
	}
	...
	if(($total_page>1) && ($record_start!=($total_page*$record_count-$record_count))) {
		...
		echo("
&nbsp;&nbsp;&nbsp;<a href='delivery_supply_list.php?record_start=$next_page_start'><font size='3'>▶</font></a>
		");
	}
}
	echo("
...
<table>
	<tr>
		<td align='center'>
			<form action='delivery_supply_list.php?record_start=$record_start' method='post'>
			<select name='field'>
